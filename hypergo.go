...
type Option func(*HyperGo) error

// HyperGo provides a service to render views from different template adapters.
type HyperGo struct {
...
// NewHyperGo creates a new view service. It accepts a list of options to configure the view service.
//
// Available options:
//
...
//   - WithLogger: sets an initial logger to use for the HyperGo instance. If not set, a default logger is created when the HyperGo instance is created.
...
func NewHyperGo(options ...Option) (*HyperGo, error) {
	hgo := &HyperGo{
...
func (s *HyperGo) Logger() *slog.Logger {
	return s.logger
}
...
func (s *HyperGo) MaybeRegisterDefaultAdapters() error {
...
func (s *HyperGo) RegisterAdapter(name string, adapter Adapter) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func (s *HyperGo) Adapter(name string) (Adapter, bool) {
	s.mu.RLock()
	defer s.mu.RUnlock()
...
func (s *HyperGo) RenderNotFound(w http.ResponseWriter, r *http.Request) {
	s.RenderNotFoundAs(w, r, "html")
}

// RenderNotFoundAs renders a 404 not found page as the specified adapter
func (s *HyperGo) RenderNotFoundAs(w http.ResponseWriter, r *http.Request, adapterKey string) {
	if adapter, ok := s.adapterFor(w, adapterKey); ok {
...
func (s *HyperGo) RenderSystemError(w http.ResponseWriter, r *http.Request, err error) {
	s.RenderSystemErrorAs(w, r, "html", err)
}

// RenderSystemErrorAs renders a system error page as the specified adapter
func (s *HyperGo) RenderSystemErrorAs(w http.ResponseWriter, r *http.Request, adapterKey string, err error) {
...
func (s *HyperGo) RenderMaintenance(w http.ResponseWriter, r *http.Request) {
	s.RenderMaintenanceAs(w, r, "html")
}

// RenderMaintenanceAs renders a maintenance page as the specified adapter
func (s *HyperGo) RenderMaintenanceAs(w http.ResponseWriter, r *http.Request, adapterKey string) {
	if adapter, ok := s.adapterFor(w, adapterKey); ok {
...
func (s *HyperGo) RenderForbidden(w http.ResponseWriter, r *http.Request) {
	s.RenderForbiddenAs(w, r, "html")
}

// RenderForbiddenAs renders a forbidden page as the specified adapter
func (s *HyperGo) RenderForbiddenAs(w http.ResponseWriter, r *http.Request, adapterKey string) {
	if adapter, ok := s.adapterFor(w, adapterKey); ok {
...
func (s *HyperGo) RenderMethodNotAllowed(w http.ResponseWriter, r *http.Request) {
	s.RenderMethodNotAllowedAs(w, r, "html")
}

// RenderMethodNotAllowedAs renders a method not allowed page as the specified adapter
func (s *HyperGo) RenderMethodNotAllowedAs(w http.ResponseWriter, r *http.Request, adapterKey string) {
	if adapter, ok := s.adapterFor(w, adapterKey); ok {
...
func (s *HyperGo) RenderUnauthorized(w http.ResponseWriter, r *http.Request) {
	s.RenderUnauthorizedAs(w, r, "html")
}

// RenderUnauthorizedAs renders an unauthorized page as the specified adapter
func (s *HyperGo) RenderUnauthorizedAs(w http.ResponseWriter, r *http.Request, adapterKey string) {
	if adapter, ok := s.adapterFor(w, adapterKey); ok {
...
func (s *HyperGo) Redirect(w http.ResponseWriter, r *http.Request, url string) {
...
func (s *HyperGo) adapterFor(w http.ResponseWriter, key string) (Adapter, bool) {
	if key == "" {
		key = "html"
	}
